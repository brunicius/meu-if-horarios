...
function getDayTitle(day) {
  if (day <= 6) return dayNames[day]
  else return dayNames[0]
}
function removeTimes() {
  let elements = document.querySelectorAll(
    '.time-list > .time, .no-time, .ct-day'
  )
...
function renderToday(update) {
  ...
  todayTitle.innerHTML = `${getDayTitle(
    now.getDay()
  ...
  if (todayData.idle) {
    ...
    timeList.innerHTML =
      '<p class="no-time time-sub-title" style="margin: 15px">Sem horários definidos hoje</p>'
    renderTomorrow()
    return
  } else timeList.dataset.idle = false

  let recreate = lastDay != now.getDay() ? true : false

  if ((timeList.children.length < 2 && !update) || recreate) {
    // Cria os horários
    ...
    for (let i = 0; i <= todayData.times.length - 1; i++) {
      ...
      timeEl.innerHTML = `
                ...
                <div class="time-hour">${timeData.startTime.substring(
                  0,
                  5
                )} - ${timeData.endTime.substring(0, 5)}</div>
                <div class="time-name">${timeData.name}</div>
                <div class="time-teacher">
                    <div class="tt-title time-sub-title">Professor (a)</div>
                    <div class="tt-name time-sup-title">${
                      timeData.teacher
                    }</div>
                </div>
                <div class="time-room">
    ...
    for (let i = 0; i <= todayData.times.length - 1; i++) {
      ...
      if (isTimeAfter(timeEl.dataset.startTime, timeEl.dataset.endTime, now)) {
        ...
        isTimeBetween(timeEl.dataset.startTime, timeEl.dataset.endTime, now)
      ) {
        timeEl.classList.add('active')
        timeEl.querySelector('.s-title').innerHTML = 'Agora'
        timeEl.querySelector('.s-title').classList.add('active')
        ...
        let startDate = new Date(
          `${now.toDateString()} ${timeEl.dataset.startTime}`
        )
        let endDate = new Date(
          `${now.toDateString()} ${timeEl.dataset.endTime}`
        )
        ...
        isTimeBefore(timeEl.dataset.startTime, timeEl.dataset.endTime, now)
      ) {
        timeEl.classList.remove('active')
        timeEl.querySelector('.s-title').innerHTML = 'Não iniciada'
        timeEl.querySelector('.pb-w').classList.add('hidden')
...
function renderTomorrow() {
  ...
  tomorrowTitle.innerHTML = `${getDayTitle(
    now.getDay()
  )}, ${now.toLocaleDateString()}`
  ...
  if (tomorrowData.idle) {
    timeList.dataset.idle = true
    timeList.innerHTML =
      '<p class="no-time time-sub-title" style="margin: 15px">Sem horários definidos para amanhã.</p>'
    return
  } else timeList.dataset.idle = false
